...
def compute_transition_audio(
    ...
    if available_a_for_crossfade < crossfade_samples:
        print(f"Warning: Song A has only {
              available_a_for_crossfade/sr:.2f}s available for crossfade, adjusting crossfade duration")
    ...
    if available_b_for_crossfade < actual_crossfade_samples:
        print(f"Warning: Song B has only {
              available_b_for_crossfade/sr:.2f}s available for crossfade, adjusting")
    ...
    actual_crossfade_samples = min(
        ...
        # Available in segment_b
        segment_b.shape[-1] - (segment_b_start - segment_b_start)
    )
    ...
    if actual_crossfade_samples > 0:
        ...
        if audio_a.ndim == 1:
            crossfade_a = segment_a[actual_pre_samples:
                                    actual_pre_samples + actual_crossfade_samples]
    ...
    if audio_b.ndim == 1:
        post_transition = segment_b[actual_crossfade_samples:
                                    actual_crossfade_samples + actual_post_samples]
    else:
        post_transition = segment_b[:,
                                    actual_crossfade_samples:actual_crossfade_samples + actual_post_samples]
...
if __name__ == "__main__":
    ...
    new_mix = compute_transition_audio(
        song_a, song_b, time_a=215.0, time_b=0.0)
